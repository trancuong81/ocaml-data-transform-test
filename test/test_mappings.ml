...
let test_textbox_mapping () =
  let input = Yojson.Safe.from_string
    {|{"subdoc": {"lp_signatory": {"asa_commitment_amount": {"value": "1000000"}}}}|}
  in
...
let test_checkbox_mapping () =
  ...
  let keys = JP.get_string_list result
    ["sf_Account_SubscriptionInvestor_WLC_Publicly_Listed_On_A_Stock_Exchange_c"; "selectedKeys"]
  in
